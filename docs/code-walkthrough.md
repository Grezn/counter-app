...
班別交接清單使用另一個 localStorage key：

```text
localStorage key: noc_shift_handoff_state
```

這個狀態只保留今日開班/收班 checklist、開班時間、收班時間與最近一次收班小結。跨日載入會自動重開一份今日狀態，不送後端。

「開班」會記錄本班開始時間，開班 checklist 用來確認 LINE、Outlook、Jira、監控入口與話機。「產生收班小結」會統計目前未結案、待確認、可結案事件，並整理開班/收班 checklist 完成度；「複製小結」會把這段班別層級文字放進剪貼簿。
...
交班前檢查列會整合目前表單與未結案紀錄，提醒摘要缺欄位、目前事件尚未儲存或更新、事件已到下次確認時間、追蹤中的事件缺下次確認、未結案事件沒有下一步，以及可結案事件。提示文字可以直接點擊，帶去補欄位、儲存事件或查看事件列表。若目前表單與未結案紀錄標題、客戶或系統相近，會另外顯示相似事件提醒，避免同一個告警被重開一筆。
...
「追蹤狀態」會顯示在交班摘要與事件卡片上。不需追蹤或可結案的事件會自動清除並停用下次確認時間，摘要顯示為不需設定；需追蹤與持續監控會要求下次確認。等客戶回覆或等二線回覆可以不填明確確認時間，但事件卡片仍會保留等待狀態。
