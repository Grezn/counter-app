...
「複製交班摘要」會先檢查一句話主旨、目前狀態、影響範圍、下一步與已通知。摘要標題旁會顯示完整度；缺欄位時不會複製，提示裡的欄位名稱可以直接點擊跳去補齊。補齊後才會把接手重點、問題影響、下一步和已完成檢查組成文字，方便貼到 Jira、交班小卡或訊息。

交班前檢查列會整合目前表單與未結案紀錄，提醒摘要缺欄位、目前事件尚未儲存或更新、事件已到下次確認時間，以及未結案事件沒有下一步。提示文字可以直接點擊，帶去補欄位、儲存事件或查看事件列表。若目前表單與未結案紀錄標題、客戶或系統相近，會另外顯示相似事件提醒，避免同一個告警被重開一筆。
